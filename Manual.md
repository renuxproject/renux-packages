# The Renux source packages manual

This article contains an exhaustive manual of how to create new source
packages for rpkg, the Renux native packaging system.
...
* The Renux source packages manual
	...
	* [RENUXPKG Triggers](#renuxpkg_triggers)
...
The `renux-packages` repository contains all the
...
to be processed by `rpkg-src`, the package builder, to generate binary packages.
The shell used by `rpkg-src` is GNU bash; `rpkg-src` doesn't aim to be
...
in a directory matching `$pkgname`, Example: `renux-packages/srcpkgs/foo/template`.

If everything went fine after running

    $ ./rpkg-src pkg <pkgname>

a binary package named `foo-1.0_1.<arch>.rpkg` will be generated in the local repository
`hostdir/binpkgs`.
...
`do_check` should be customized to limit testsuite unless `RENUXPKG_CHECK_PKGS` is `full`.
...
`rpkg-src` supports running just the specified phase, and if it ran
successfully, the phase will be skipped later (unless its work directory
`${wrksrc}` is removed with `rpkg-src clean`).
...
in contrast to many other distributions, renux doesn't lowercase package names.
...
The following functions are defined by `rpkg-src` and can be used on any template:
...
- *vsv()* `vsv <service> [<facility>]`
	...
	see [vlogger(8)](https://man.renux.org/vlogger.8).
...
- *vsrcextract()* `[-C <target directory>] [--no-strip-components|--strip-components=<n>] <file>`
	...
	Extracts `$RENUXPKG_SRCDISTDIR/$pkgname-$version/<file>` to `target directory`
...
- *vsrccopy()* `<file>... <target>`
	...
	Copies `file`s from `$RENUXPKG_SRCDISTDIR/$pkgname-$version/` into the `target` directory,
...
The following variables are defined by `rpkg-src` and can be used on any template:

- `makejobs` Set to `-jX` if `RENUXPKG_MAKEJOBS` is defined, to allow parallel jobs with `GNU make`.
...
- `CROSS_BUILD` Set if `rpkg-src` is cross compiling a package.

- `RENUXPKG_CHECK_PKGS` Set if `rpkg-src` is going to run tests for a package.
...
- `RENUXPKG_BUILDDIR` Directory to store the `source code` of the source package being processed,
set to `<masterdir>/builddir`. The package `wrksrc` is always stored
in this directory such as `${RENUXPKG_BUILDDIR}/${wrksrc}`.

- `RENUXPKG_MACHINE` The machine architecture as returned by `rpkg-uhelper arch`.

- `RENUXPKG_ENDIAN` The machine's endianness ("le" or "be").

- `RENUXPKG_LIBC` The machine's C library ("glibc" or "musl").

- `RENUXPKG_WORDSIZE` The machine's word size in bits (32 or 64).

- `RENUXPKG_NO_ATOMIC8` The machine lacks native 64-bit atomics (needs libatomic emulation).

- `RENUXPKG_SRCDISTDIR` Full path to where the `source distfiles` are stored, i.e `$RENUXPKG_HOSTDIR/sources`.

- `RENUXPKG_SRCPKGDIR` Full path to the `srcpkgs` directory.

- `RENUXPKG_TARGET_MACHINE` The target machine architecture when cross compiling a package.

- `RENUXPKG_TARGET_ENDIAN` The target machine's endianness ("le" or "be").

- `RENUXPKG_TARGET_LIBC` The target machine's C library ("glibc" or "musl").

- `RENUXPKG_TARGET_WORDSIZE` The target machine's word size in bits (32 or 64).

- `RENUXPKG_TARGET_NO_ATOMIC8` The target machine lacks native 64-bit atomics (needs libatomic emulation).

- `RENUXPKG_FETCH_CMD` The utility to fetch files from `ftp`, `http` of `https` servers.

- `RENUXPKG_WRAPPERDIR` Full path to where rpkg-src's wrappers for utilities are stored.

- `RENUXPKG_CROSS_BASE` Full path to where cross-compile dependencies are installed, varies according to the target architecture triplet. i.e `aarch64` -> `/usr/aarch64-linux-gnu`.

- `RENUXPKG_RUST_TARGET` The target architecture triplet used by `rustc` and `cargo`.

- `RENUXPKG_BUILD_ENVIRONMENT` Enables continuous-integration-specific operations. Set to `renux-packages-ci` if in continuous integration.
...
and `RENUXPKG_MAKEJOBS` will be set to 1. If a package does not work well with `RENUXPKG_MAKEJOBS`
but still has a mechanism to build in parallel, set `disable_parallel_build` and
use `RENUXPKG_ORIG_MAKEJOBS` (which holds the original value of `RENUXPKG_MAKEJOBS`) in the template.
...
Allowed values:
  - `yes` (the default) to run if `RENUXPKG_CHECK_PKGS` is set.
  - `extended` to run if `RENUXPKG_CHECK_PKGS` is `full`.
  - `ci-skip` to run locally if `RENUXPKG_CHECK_PKGS` is set, but not as part of pull request checks.
  - `no` to never run.
...
- `nodebug` If enabled -dbg packages won't be generated even if `RENUXPKG_DEBUG_PKGS` is set.
...
- `restricted` If set, rpkg-src will refuse to build the package unless
`etc/conf` has `RENUXPKG_ALLOW_RESTRICTED=yes`. The primary builders for Void
...
- `reverts` renuxpkg supports a unique feature which allows to downgrade from broken
...
It is used in the `x11-fonts` rpkg-trigger to rebuild the font cache during install/removal
of the package.
...
that will be installed and removed by the `dkms` rpkg-trigger with the install/removal of the
package.
...
package metadata and can be queried with `rpkg-query` by users.
...
- `nocheckperms` If set, rpkg-src will not fail on common permission errors (world writable files, etc.)

- `nofixperms` If set, rpkg-src will not fix common permission errors (executable manpages, etc.)

- `no_generic_pkgconfig_link` If set, rpkg-src will not create a symlink from `$RENUXPKG_CROSS_TRIPLET-pkg-config`
  to `$RENUXPKG_WRAPPERDIR/pkg-config` before building the template.
...
are necessary because `rpkg-src` supports cross compilation and to avoid
...
Furthermore, if `RENUXPKG_CHECK_PKGS` is set or the `-Q` option is passed to
`rpkg-src`, the target package might require specific dependencies or libraries
...
rpkg, should be listed in `depends`.

Libraries linked by ELF objects are detected automatically by `rpkg-src`, hence they
...
only checked if they exist as binary packages, and are built automatically by `rpkg-src` if
...
(e.g., `depends="virtual?vpkg-0.1_1"`). When a package is built by `rpkg-src`, providers for any
...
* `bootstrap`: Repository for rpkg-src-specific packages.
...
`./rpkg-src update-check <pkgname>`. In some cases you need to override
...
To handle this, rpkg-src has patching functionality. It will look for all files
...
rpkg-src will only apply patches named in the `series` file.
...
`renux-packages/common/build-style` directory. Please note that required packages
...
- `renux-cross` For cross-toolchain packages used to build Void systems. There are no
...
- `meson` creates a cross file, `${RENUXPKG_WRAPPERDIR}/meson/renuxpkg_meson.cross`, which configures
...
`$RENUXPKG_SRCDISTDIR/$pkgname-$version/` so libraries specified as meson wraps can be added to
...
configured, a secondary cross file, `${RENUXPKG_WRAPPERDIR}/meson/renuxpkg_numpy.cross`, will be
...
to `qemu-<target_arch>-static` and `QEMU_LD_PREFIX` to `RENUXPKG_CROSS_BASE`.
...
- For do_fetch, post_fetch: `RENUXPKG_BUILDDIR`.
...
additional features; The Renux source packages collection allows you to do this with some simple tweaks
...
The supported build options for a source package can be shown with `rpkg-src`:

    $ ./rpkg-src show-options foo

Build options can be enabled with the `-o` flag of `rpkg-src`:

    $ ./rpkg-src -o option,option1 <cmd> foo
...
at the same time with `rpkg-src`:
    ...
    $ ./rpkg-src -o option,~option1,~option2 <cmd> foo

The build options can also be shown for binary packages via `rpkg-query(8)`:
...
Example: `RENUXPKG_PKG_OPTIONS_xorg_server=opt`.
...
that are always set by `renuxpkg` when the scripts are executed:
...
- `$CONF_FILE`: full path to `renuxpkg.conf`.
...
those detected automatically by `rpkg-src` and **depends**. Please note that
...
- `RENUXPKG_MAKEJOBS`: Value passed to the `-p` flag of `go install`, to
...
<a id="renuxpkg_triggers"></a>
### rpkg Triggers

rpkg triggers are a collection of snippets of code, provided by the `rpkg-triggers`
...
See [`update-binfmts(8)`](https://man.renux.org/man8/update-binfmts.8) for more details.
...
Although `rpkg-src` will automatically include the `dkms` trigger whenever
...
`srcpkgs/<pkgname>/files/README.renux` and install with
`vdoc "${FILESDIR}/README.renux"`.
...
the installed files (`./rpkg-src show-files pkg`) before pushing new updates.

- Make sure that binaries are not stripped by the software, let rpkg-src do this;
otherwise the `debug` packages won't have debugging symbols.

<a id="contributing"></a>
### Contributing via git

To get started, [fork](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/working-with-forks/fork-a-repo) the renuxproject `renux-packages` git repository on GitHub and clone it:

    $ git clone git@github.com:<user>/renux-packages.git
...
to pull in new changes:
    ...
    $ git remote add upstream https://github.com/renuxproject/renux-packages.git
...
us at `#renuxpkg` via IRC at `irc.libera.chat`.
