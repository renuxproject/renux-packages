# Contributing to renux-packages

> [!IMPORTANT]
> Renux follows Void's [contributing policy](https://github.com/void-linux/.github/blob/master/CONTRIBUTING.md) applies to this repository

renux-packages is the backbone of the Void Linux distribution. It contains all the definitions to build packages from source.

This document describes how you, as a contributor, can help with adding packages, correcting bugs and adding features to renux-packages.
...
There's also a [manual for rpkg-src](./README.md), which is used to build package files from templates.
...
To get started, [fork](https://help.github.com/articles/fork-a-repo) the renuxproject `renux-packages` git repository on GitHub and clone it:
...
This can also be done with the `github-cli` tool:
    ...
    $ gh repo fork renuxproject/renux-packages
    $ gh repo clone <user>/renux-packages
...
Templates must have the name `renux-packages/srcpkgs/<pkgname>/template`, where `pkgname` is the same as the `pkgname` variable in the template.
...
A good way to do this is to work on orphaned packages (those maintained by `orphan@renux.org`) that you use.
...
If a template is orphaned (maintained by `orphan@renux.org`) or the current `maintainer` has not contributed to
...
the template to `Orphaned <orphan@renux.org>`. The commit message should mention this, for example:
...
After making your changes, please check that the package builds successfully. From the top level directory of your local copy of the `renux-packages` repository, run:
...
Your package must build successfully for at least x86, but we recommend also trying a cross-build for armv6l* as well, e.g.:
    ...
    $ ./rpkg-src -a armv6l pkg <pkgname>

When building for `x86_64*` or `i686`, building with the `-Q` flag or with `RENUXPKG_CHECK_PKGS=yes` set in `etc/conf` (to run the check phase) is strongly encouraged.
...
* for new packages, use `New package: <pkgname>-<version>` ([example](https://github.com/renuxproject/renux-packages/commit/8ed8d41c40bf6a82cf006c7e207e05942c15bff8)).

* for package updates, use `<pkgname>: update to <version>.` ([example](https://github.com/renuxproject/renux-packages/commit/c92203f1d6f33026ae89f3e4c1012fb6450bbac1)).

* for template modifications without a version change, use `<pkgname>: <reason>` ([example](https://github.com/renuxproject/renux-packages/commit/ff39c912d412717d17232de9564f659b037e95b5)).

* for package removals, use `<pkgname>: remove package` and include the removal reason in the commit body ([example](https://github.com/renuxproject/renux-packages/commit/4322f923bdf5d4e0eb36738d4f4717d72d0a0ca4)).

* for changes to any other file, use `<filename>: <reason>` ([example](https://github.com/renuxproject/renux-packages/commit/e00bea014c36a70d60acfa1758514b0c7cb0627d),
  [example](https://github.com/renuxproject/renux-packages/commit/93bf159ce10d8e474da5296e5bc98350d00c6c82), [example](https://github.com/renuxproject/renux-packages/commit/dc62938c67b66a7ff295eab541dc37b92fb9fb78), [example](https://github.com/renuxproject/renux-packages/commit/e52317e939d41090562cf8f8131a68772245bdde))

If you want to describe your changes in more detail, explain in the commit body (separated from the first line with a blank line) ([example](https://github.com/renuxproject/renux-packages/commit/f1c45a502086ba1952f23ace9084a870ce437bc6)).
...
Most pull requests should only contain a single package and dependencies which are not part of renux-packages yet.
...
Use `./rpkg-src -Q pkg <package>` to do so.
...
Once the reviewers have merged the pull request, our [build server](http://build.renux.org) is automatically triggered and builds
...
First, [clone the repository](https://github.com/renuxproject/renux-packages#quick-start) if you haven't done so already.
...
If your local renux-packages repository is cloned from your fork, you may need to add the main repository as a remote first:
...
Then [build and install](https://github.com/renuxproject/renux-packages#building-packages) the package and test its functionality.
