...
do_build() {
	...
	if [ "$CROSS_BUILD" ]; then
		...
		_TARGET_PLATFORM="$(ruby -r \
			$(find ${RENUXPKG_CROSS_BASE}/usr/lib/ruby -iname rbconfig.rb) \
...
system("sed -i 's|^topdir.*|topdir = ${RENUXPKG_CROSS_BASE}/usr/include/ruby-\$(ruby_version)|' Makefile")
system("sed -i 's|^hdrdir.*|hdrdir = ${RENUXPKG_CROSS_BASE}/usr/include/ruby-\$(ruby_version)|' Makefile")
system("sed -i 's|^arch_hdrdir.*|arch_hdrdir = ${RENUXPKG_CROSS_BASE}/usr/include/ruby-\$(ruby_version)/\$(arch)|' Makefile")
...
EOF
	...
	for f in $distfiles; do
		if [ "${f##*.}" = "gem" ]; then
			$gem_cmd spec \
				"${RENUXPKG_SRCDISTDIR}/${pkgname}-${version}/${f##*/}" \
				--ruby > $gemspec
		fi
	done
