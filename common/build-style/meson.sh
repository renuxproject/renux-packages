...
do_configure() {
	...
	: ${meson_crossfile:="${RENUXPKG_WRAPPERDIR}/meson/renuxpkg_meson.cross"}
	...
	${meson_cmd} setup \
		--prefix=/usr \
		--libdir=/usr/lib${RENUXPKG_TARGET_WORDSIZE} \
...
do_build() {
	...
	${make_cmd} -C ${meson_builddir} ${RENUXPKG_VERBOSE+${make_verbose}} ${makejobs} ${make_build_args} ${make_build_target}
}
...
do_check() {
	...
	${make_check_pre} ${make_cmd} ${make_check_target} ${RENUXPKG_VERBOSE+${make_verbose}} -C ${meson_builddir} ${makejobs} ${make_check_args}
}
...
do_install() {
	...
	DESTDIR=${DESTDIR} ${make_cmd} ${RENUXPKG_VERBOSE+${make_verbose}} -C ${meson_builddir} ${make_install_args} ${make_install_target}
}
