do_build() {
	...
	# avoid it.
	if [ "$CROSS_BUILD" ]; then
		zig_target="${RENUXPKG_CROSS_ZIG_TARGET}"
		zig_cpu="${RENUXPKG_CROSS_ZIG_CPU}"
	else
		zig_target="${RENUXPKG_ZIG_TARGET}"
		zig_cpu="${RENUXPKG_ZIG_CPU}"
	fi

	# Inform zig of the required libc include paths.
	cat > renuxpkg_zig_libc.txt <<-EOF
		include_dir=${RENUXPKG_CROSS_BASE}/usr/include
		sys_include_dir=${RENUXPKG_CROSS_BASE}/usr/include
		crt_dir=${RENUXPKG_CROSS_BASE}/usr/lib
		msvc_lib_dir=
		kernel32_lib_dir=
		gcc_dir=
	EOF
	...
	DESTDIR="zig-out" zig build \
		...
		--search-prefix "${RENUXPKG_CROSS_BASE}/usr" \
		--prefix /usr \
		--global-cache-dir /host/zig \
		--libc renuxpkg_zig_libc.txt \
		--release=safe \
		--verbose \
