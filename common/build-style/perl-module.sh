...
# than $RENUXPKG_BUILDDIR/$wrksrc/$build_wrksrc, one can use (relative to
# $wrksrc/$build_wrksrc):
#
...
do_configure() {
	...
	local perlprefix=${RENUXPKG_STATEDIR}/perlprefix-${RENUXPKG_TARGET_MACHINE}
	mkdir -p $perlprefix
	if [ -d "$RENUXPKG_CROSS_BASE/usr/lib/perl5/core_perl" ]; then
		cp "$RENUXPKG_CROSS_BASE/usr/lib/perl5/core_perl/Config"*.p? $perlprefix
		cp "$RENUXPKG_CROSS_BASE/usr/lib/perl5/core_perl/Errno.pm" $perlprefix
		sed -i -e "s;archlibexp => '\(.*\)';archlibexp => '${RENUXPKG_CROSS_BASE}\1';" \
	...
	if [ -f "${wrksrc}/${build_wrksrc:+$build_wrksrc/}Makefile.PL" ]; then
		sed -i "s,/usr/include,${RENUXPKG_CROSS_BASE}/usr/include,g" \
	...
	if [ -z "$perl_configure_dirs" ]; then
		...
		PERL_MM_USE_DEFAULT=1 GCC="$CC" CC="$CC" LD="$CC" \
			OPTIMIZE="$CFLAGS" \
			CFLAGS="$CFLAGS -I${RENUXPKG_CROSS_BASE}/usr/include" \
			LDFLAGS="$LDFLAGS -L${RENUXPKG_CROSS_BASE}/usr/lib -lperl" \
			LDDLFLAGS="-shared $CFLAGS -L${RENUXPKG_CROSS_BASE}/usr/lib" \
	...
	for i in ${perl_configure_dirs}; do
		...
		if [ -f "$perlmkf" ]; then
			...
			PERL_MM_USE_DEFAULT=1 GCC="$CC" CC="$CC" LD="$CC" \
				...
				CFLAGS="$CFLAGS -I${RENUXPKG_CROSS_BASE}/usr/include" \
				LDFLAGS="$LDFLAGS -L${RENUXPKG_CROSS_BASE}/usr/lib -lperl" \
				LDDLFLAGS="-shared $CFLAGS -L${RENUXPKG_CROSS_BASE}/usr/lib -lperl" \
