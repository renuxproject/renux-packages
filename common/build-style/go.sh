...
do_build() {
	# remove -s and -w from go_ldflags, we should let rpkg-src strip binaries itself
	...
	if [ "${go_mod_mode}" != "off" ] && [ -f go.mod ]; then
		...
		go install -p "$RENUXPKG_MAKEJOBS" -mod="${go_mod_mode}" -modcacherw -v -tags "${go_build_tags}" -ldflags "${go_ldflags}" ${go_package}
		if [[ -n "${_go_mod_path}" ]]; then
			popd
		fi
	else
		# Otherwise, build using GOPATH
		go install -p "$RENUXPKG_MAKEJOBS" -v -tags "${go_build_tags}" -ldflags "${go_ldflags}" ${go_package}
	fi
}
...
do_check() {
	...
	${make_check_pre} go test -p "$RENUXPKG_MAKEJOBS" -tags "${go_build_tags}" -ldflags "${go_ldflags}" ${make_check_args} ${make_check_target}
}
