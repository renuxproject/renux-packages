...
do_build() {
	...
	if [ "${CROSS_BUILD}" ] && [[ "${build_helper}" = *meson* ]]; then
		local mcross="-Csetup-args=--cross-file=${RENUXPKG_WRAPPERDIR}/meson"
		make_build_args+=" ${mcross}/renuxpkg_meson.cross"

		if [[ "${build_helper}" = *numpy* ]]; then
			make_build_args+=" ${mcross}/renuxpkg_numpy.cross"
		fi
	fi
...
do_check() {
	...
	if python3 -c 'import xdist' >/dev/null 2>&1; then
		testjobs="-n $RENUXPKG_MAKEJOBS"
	fi

	local testdir="${wrksrc}/.rpkg-testdir/$(date +%s)"
