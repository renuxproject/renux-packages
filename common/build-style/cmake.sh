...
SET(CMAKE_FIND_ROOT_PATH  "${RENUXPKG_MASTERDIR}/usr;${RENUXPKG_MASTERDIR}")
...
_EOF
		...
		cat > cross_${RENUXPKG_CROSS_TRIPLET}.cmake <<_EOF
SET(CMAKE_SYSTEM_NAME Linux)
SET(CMAKE_SYSTEM_VERSION 1)

SET(CMAKE_C_COMPILER   ${CC})
SET(CMAKE_CXX_COMPILER ${CXX})
SET(Rust_CARGO_TARGET ${RENUXPKG_CROSS_RUST_TARGET})
SET(CMAKE_CROSSCOMPILING TRUE)

SET(CMAKE_SYSTEM_PROCESSOR ${_CMAKE_SYSTEM_PROCESSOR})

SET(CMAKE_FIND_ROOT_PATH  "${RENUXPKG_CROSS_BASE}/usr;${RENUXPKG_CROSS_BASE}")

SET(CMAKE_FIND_ROOT_PATH_MODE_PROGRAM NEVER)
SET(CMAKE_FIND_ROOT_PATH_MODE_LIBRARY ONLY)
SET(CMAKE_FIND_ROOT_PATH_MODE_INCLUDE ONLY)
_EOF
		cmake_args+=" -DCMAKE_TOOLCHAIN_FILE=${wrksrc}/${build_wrksrc}/${cmake_builddir}/cross_${RENUXPKG_CROSS_TRIPLET}.cmake"
	fi
	cmake_args+=" -DCMAKE_INSTALL_PREFIX:PATH=/usr"
	cmake_args+=" -DCMAKE_BUILD_TYPE=None"
	cmake_args+=" -DCMAKE_INSTALL_LIBDIR:PATH=lib${RENUXPKG_TARGET_WORDSIZE}"
	...
	if [[ $build_helper = *"qemu"* ]]; then
		echo "SET(CMAKE_CROSSCOMPILING_EMULATOR /usr/bin/qemu-${RENUXPKG_TARGET_QEMU_MACHINE}-static)" \
			>> cross_${RENUXPKG_CROSS_TRIPLET}.cmake
	fi
	...
	CFLAGS="-DNDEBUG ${CFLAGS/ -pipe / }" CXXFLAGS="-DNDEBUG ${CXXFLAGS/ -pipe / }" \
		cmake ${RENUXPKG_VERBOSE--Wno-dev} ${cmake_args} ${configure_args} \
...
do_build() {
	...
	${make_cmd} ${makejobs} ${RENUXPKG_VERBOSE+${make_verbose}} ${make_build_args} ${make_build_target}
}
...
do_check() {
	...
	${make_check_pre} ${make_cmd} ${makejobs} ${RENUXPKG_VERBOSE+${make_verbose}} ${make_check_args} ${make_check_target}
}
...
do_install() {
	...
	cd ${cmake_builddir:=build}
	DESTDIR=${DESTDIR} ${make_cmd} ${RENUXPKG_VERBOSE+${make_verbose}} ${make_install_args} ${make_install_target}
}
