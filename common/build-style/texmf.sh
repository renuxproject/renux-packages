do_build() {
	...
	done < "${RENUXPKG_COMMONDIR}/environment/build-style/texmf/ownership.txt"
}
...
do_check() {
	...
	while read p; do
		...
		current_ver="$(grep -m 1 version= ${RENUXPKG_SRCPKGDIR}/${p%-*}/template | cut -d= -f2)"
		current_rev="$(grep -m 1 revision= ${RENUXPKG_SRCPKGDIR}/${p%-*}/template | cut -d= -f2)"
		...
		done < <(rpkg-query -Rf $p | sed 's/ -> .*//')
	done < <(rpkg-query -Rs texlive -p pkgver | cut -d : -f 1)
	return $exitcode
}
