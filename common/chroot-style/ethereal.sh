...
# 1. Symlink / to masterdir inside the renux-packages repo
# 2. write the arch of the host system, as dictated by rpkg-uhelper arch
# into /.rpkg_chroot_init
#
...
# Docker example:
# $ mkdir -p /tmp/image
# $ rpkg-install -y -r /tmp/image \
#				 -R http://mirrors.servercentral.com/renux/current \
#				 -S base-chroot
# $ tar -pC /tmp/image -c . | sudo docker import - renux/masterdir
# $ rm -rf /tmp/image
# # docker run --rm -it \
#			   -e RENUXPKG_CHROOT_CMD=ethereal \
#			   -e RENUXPKG_ALLOW_CHROOT_BREAKOUT=yes \
#			   -v $(pwd):/hostrepo renux/masterdir \
#			   /bin/bash -c 'ln -s / /hostrepo/masterdir && /hostrepo/rpkg-src pkg <pkgname>'
#
...
if [ "${RENUXPKG_ALLOW_CHROOT_BREAKOUT}" != "yes" ]; then
	msg_red "chroot-style 'ethereal' requires RENUXPKG_ALLOW_CHROOT_BREAKOUT=yes\n"
...
fake_mount "$DISTDIR" "$MASTERDIR"/renux-packages
...
# rpkg-src may send some other binds, parse them here
...
# Tell rpkg-src that we are "in the chroot"
...
rm -f "$MASTERDIR"/renux-packages
mkdir -p "$MASTERDIR"/renux-packages
