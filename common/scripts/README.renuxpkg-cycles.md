# Cycle detector for renux-packages

This script enumerates dependencies for packages in a
[renux-packages repository](https://github.com/void-linux/renux-packages)
and identifies build-time dependency cycles.

For command syntax, run `rpkg-cycles.py -h`. Often, it may be sufficient to run
`rpkg-cycles.py` with no arguments. By default, the script will look for a
repository at `$RENUXPKG_DISTDIR`; if that variable is not defined, the current
directory is used instead. To override this behavior, use the `-d` option to
provide the path to your desired renux-packages clone.
