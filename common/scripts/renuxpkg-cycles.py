...
def enum_depends(pkg, renuxpkgsdir, cachedir):
	'''
	Return a pair (pkg, [dependencies]), where [dependencies] is the list
	of dependencies for the given package pkg. The argument renuxpkgsdir should
	be a path to a renux-packages repository. Dependencies will be
	determined by invoking

		<renuxpkgsdir>/rpkg-src show-build-deps <pkg>
	...
	cmd = [os.path.join(renuxpkgsdir, 'rpkg-src'), 'show-build-deps', pkg]

	try:
		deps = subprocess.check_output(cmd)
	except subprocess.CalledProcessError as err:
		print('rpkg-src failed to find dependencies for package', pkg)
		deps = [ ]
	else:
...
def find_cycles(depmap, renuxpkgsdir):
	'''
	For a map depmap: package -> [dependencies], construct a directed graph
	and identify any cycles therein.

	The argument renuxpkgsdir should be a path to the root of a renux-packages
	repository. All package names in depmap will be appended to the path
	<renuxpkgsdir>/srcpkgs and reduced with os.path.realpath to coalesce
	subpackages.
	'''
	G = nx.DiGraph()

	for i, deps in depmap.items():
		path = os.path.join(renuxpkgsdir, 'srcpkgs', i)
		i = os.path.basename(os.path.realpath(path))

		for j in deps:
			path = os.path.join(renuxpkgsdir, 'srcpkgs', j.strip())
...
if __name__ == '__main__':
	parser = ArgumentParser(description='Cycle detector for rpkg-src')
	...
	parser.add_argument('-d', '--directory',
			default=None, help='Path to renux-packages repo')
	...
	if not args.directory:
		try: args.directory = os.environ['RENUXPKG_DISTDIR']
		except KeyError: args.directory = '.'

	if args.check_pkgs:
		os.environ['RENUXPKG_CHECK_PKGS'] = args.check_pkgs
