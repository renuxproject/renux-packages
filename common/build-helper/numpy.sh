...
CFLAGS+=" -I${RENUXPKG_CROSS_BASE}/${py3_sitelib}/numpy/_core/include"
LDFLAGS+=" -L${RENUXPKG_CROSS_BASE}/${py3_sitelib}/numpy/_core/lib"
LDFLAGS+=" -L${RENUXPKG_CROSS_BASE}/${py3_sitelib}/numpy/random/lib"

# distutils from python3-numpy looks to environment variables F77 and
# F90 rather than the rpkg-set FC
export F77="${FC}"
export F90="${FC}"
...
if _gfortran=$(command -v "${FC}"); then
	ln -sf "${_gfortran}" "${RENUXPKG_WRAPPERDIR}/gfortran"
fi
unset _gfortran

# Write a secondary meson cross file for numpy configuration
if [[ "${build_helper}" = *meson* ]]; then
	_npy_meson_cross="${RENUXPKG_WRAPPERDIR}/meson/renuxpkg_numpy.cross"
	_cross_py_site="${RENUXPKG_CROSS_BASE}/${py3_sitelib}"

	if [ ! -e "${_npy_meson_cross}" ] || [ -n "$RENUXPKG_BUILD_FORCEMODE" ]; then
