...
export MESON_PACKAGE_CACHE_DIR="${RENUXPKG_SRCDISTDIR}/${pkgname}-${version}/"

# Action is only taken for cross builds
[ -z "$CROSS_BUILD" ] && return 0

# The cross file should only be written once, unless forced
[ -e "${RENUXPKG_WRAPPERDIR}/meson/renuxpkg_meson.cross" ] && [ -z "$RENUXPKG_BUILD_FORCEMODE" ] && return 0

mkdir -p "${RENUXPKG_WRAPPERDIR}/meson"

_MESON_TARGET_ENDIAN=little
# drop the -musl suffix to the target cpu, meson doesn't recognize it
_MESON_TARGET_CPU=${RENUXPKG_TARGET_MACHINE/-musl/}
...
if [[ "${build_helper}" = *qemu* ]]; then
	_MESON_EXE_WRAPPER="exe_wrapper = '/usr/bin/qemu-${RENUXPKG_TARGET_QEMU_MACHINE}'"
fi
...
cat > "${RENUXPKG_WRAPPERDIR}/meson/renuxpkg_meson.cross" <<-EOF
	[binaries]
	${_MESON_EXE_WRAPPER:-# exe_wrapper is not set}
	c = '${CC}'
	cpp = '${CXX}'
	ar = '${RENUXPKG_CROSS_TRIPLET}-gcc-ar'
	nm = '${NM}'
	strip = '${STRIP}'
	readelf = '${READELF}'
	objcopy = '${OBJCOPY}'
	pkg-config = '${PKG_CONFIG}'
	rust = ['rustc', '--target', '${RUST_TARGET}' ,'--sysroot', '${RENUXPKG_CROSS_BASE}/usr']
	g-ir-scanner = '${RENUXPKG_CROSS_BASE}/usr/bin/g-ir-scanner'
	g-ir-compiler = '${RENUXPKG_CROSS_BASE}/usr/bin/g-ir-compiler'
	g-ir-generate = '${RENUXPKG_CROSS_BASE}/usr/bin/g-ir-generate'
	llvm-config = 'llvm-config-qemu'
	cups-config = '${RENUXPKG_CROSS_BASE}/usr/bin/cups-config'
	qmake6 = 'qmake6'
	qmake5 = 'qmake5'

	[properties]
	needs_exe_wrapper = true
	bindgen_clang_arguments = ['-target', '${RENUXPKG_CROSS_TRIPLET}']
