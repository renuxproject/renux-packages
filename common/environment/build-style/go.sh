if [ -z "$hostmakedepends" -o "${hostmakedepends##*gcc-go-tools*}" ]; then
	...
	if [ "$CROSS_BUILD" ]; then
		...
		export GCCGO="${RENUXPKG_CROSS_TRIPLET}-gccgo"
	fi
fi
...
export GOPATH="${wrksrc}/_build-${pkgname}-renuxpkg"
...
case "$RENUXPKG_TARGET_MACHINE" in
	*-musl) export GOCACHE="${RENUXPKG_HOSTDIR}/gocache-muslc" ;;
	*)	export GOCACHE="${RENUXPKG_HOSTDIR}/gocache-glibc" ;;
esac
