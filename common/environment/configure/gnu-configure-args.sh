...
. ${RENUXPKG_COMMONDIR}/build-profiles/${RENUXPKG_MACHINE}.sh
export configure_args+=" --host=$RENUXPKG_TRIPLET --build=$RENUXPKG_TRIPLET"
...
export configure_args+=" --libdir=\${exec_prefix}/lib${RENUXPKG_TARGET_WORDSIZE}"

_AUTOCONFCACHEDIR=${RENUXPKG_COMMONDIR}/environment/configure/autoconf_cache
...
export configure_args+=" --host=$RENUXPKG_CROSS_TRIPLET --with-sysroot=$RENUXPKG_CROSS_BASE --with-libtool-sysroot=$RENUXPKG_CROSS_BASE "
