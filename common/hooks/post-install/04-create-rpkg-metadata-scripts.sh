# This hook generates RENUXPKG pkg metadata INSTALL/REMOVE scripts.
...
# \$3 = VERSION
# \$4 = UPDATE	[yes/no]
# \$5 = CONF_FILE (path to renuxpkg.conf)
# \$6 = ARCH (uname -m)
#
...
TRIGGERSDIR="./usr/libexec/rpkg-triggers"
ACTION="\$1"
PKGNAME="\$2"
VERSION="\$3"
UPDATE="\$4"
...
_EOF
	...
	if [ -n "$triggers" ]; then
		...
		echo "pre)" >> $tmpf
		for f in ${triggers}; do
			if [ ! -f $RENUXPKG_TRIGGERSDIR/$f ]; then
				rm -f $tmpf
		...
		for f in ${triggers}; do
			targets=$($RENUXPKG_TRIGGERSDIR/$f targets)
			for j in ${targets}; do
...
hook() {
	...
	if [ -n "${sourcepkg}" -a "${sourcepkg}" != "${pkgname}" ]; then
		# subpkg
		meta_install=${RENUXPKG_SRCPKGDIR}/${pkgname}/${pkgname}.INSTALL
		msg_install=${RENUXPKG_SRCPKGDIR}/${pkgname}/${pkgname}.INSTALL.msg
		meta_remove=${RENUXPKG_SRCPKGDIR}/${pkgname}/${pkgname}.REMOVE
		msg_remove=${RENUXPKG_SRCPKGDIR}/${pkgname}/${pkgname}.REMOVE.msg
	else
		# sourcepkg
		meta_install=${RENUXPKG_SRCPKGDIR}/${pkgname}/INSTALL
		msg_install=${RENUXPKG_SRCPKGDIR}/${pkgname}/INSTALL.msg
		meta_remove=${RENUXPKG_SRCPKGDIR}/${pkgname}/REMOVE
		msg_remove=${RENUXPKG_SRCPKGDIR}/${pkgname}/REMOVE.msg
	fi
