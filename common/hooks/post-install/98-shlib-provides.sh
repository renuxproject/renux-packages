...
#	- generates shlib-provides file for rpkg-create(1)
...
collect_sonames() {
	...
	if [ -s "${_tmpfile}" ]; then
		tr '\n' ' ' < "${_tmpfile}" > "${RENUXPKG_STATEDIR}/${pkgname}${_suffix}-shlib-provides"
		echo >> "${RENUXPKG_STATEDIR}/${pkgname}${_suffix}-shlib-provides"
		if [ "$_mainpkg" ]; then
...
hook() {
	local _destdir32=${RENUXPKG_DESTDIR}/${pkgname}-32bit-${version}
	local _mainpkg=yes
	local _pkg
