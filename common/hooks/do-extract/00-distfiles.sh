# This hook extracts $distfiles into $RENUXPKG_BUILDDIR if $distfiles and $checksum
# variables are set.
...
hook() {
	...
	for f in ${distfiles}; do
		...
		if [ ! -f $srcdir/$curfile ]; then
			msg_error "$pkgver: cannot find ${curfile}, use 'rpkg-src fetch' first.\n"
		fi
	done
	...
	[ -z "$TAR_CMD" ] && msg_error "rpkg-src: no suitable tar cmd (bsdtar, tar)\n"

	extractdir=$(mktemp -d "$RENUXPKG_BUILDDIR/.extractdir-XXXXXXX") ||
