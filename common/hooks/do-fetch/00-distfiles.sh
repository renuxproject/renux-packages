...
contents_cksum() {
	...
	*) msg_error "$pkgver: unknown distfile suffix for $curfile.\n";;
	...
	tar|txz|tbz|tlz|tgz|crate)
		cksum=$($RENUXPKG_DIGEST_CMD <($TAR_CMD -x -O -f "$curfile"))
		if [ $? -ne 0 ]; then
			msg_error "$pkgver: extracting $curfile to pipe.\n"
		fi
		;;
	gz)
		cksum=$($RENUXPKG_DIGEST_CMD <(gunzip -c "$curfile"))
		;;
	bz2)
		cksum=$($RENUXPKG_DIGEST_CMD <(bunzip2 -c "$curfile"))
		;;
	zip)
		if command -v unzip &>/dev/null; then
			cksum=$($RENUXPKG_DIGEST_CMD <(unzip -p "$curfile"))
			if [ $? -ne 0 ]; then
				msg_error "$pkgver: extracting $curfile to pipe.\n"
			fi
		else
	...
	rpm)
		...
		;;
	txt)
		cksum=$($RENUXPKG_DIGEST_CMD "$curfile")
		;;
	7z)
		if command -v 7z &>/dev/null; then
			cksum=$($RENUXPKG_DIGEST_CMD <(7z x -o "$curfile"))
			if [ $? -ne 0 ]; then
				msg_error "$pkgver: extracting $curfile to pipe.\n"
			fi
		else
		...
		;;
	gem)
		cksum=$($RENUXPKG_DIGEST_CMD <($TAR_CMD -x -O -f "$curfile" data.tar.gz | $TAR_CMD -xzO ))
		;;
	*)
...
verify_cksum() {
	...
	if [ "${cksum:0:1}" = "@" ]; then
		...
		filesum=$(${RENUXPKG_DIGEST_CMD} "$distfile")
		if [ "$cksum" != "$filesum" ]; then
			echo
			msg_red "SHA256 mismatch for '${curfile}:'\n${filesum}\n"
			errors=$((errors + 1))
		else
			if [ ! -f "$RENUXPKG_SRCDISTDIR/by_sha256/${cksum}_${curfile}" ]; then
				mkdir -p "$RENUXPKG_SRCDISTDIR/by_sha256"
				ln -f "$distfile" "$RENUXPKG_SRCDISTDIR/by_sha256/${cksum}_${curfile}"
			fi
			msg_normal_append "OK.\n"
		fi
	fi
...
link_cksum() {
	...
	if [ -n "$cksum" -a -f "$RENUXPKG_SRCDISTDIR/by_sha256/${cksum}_${curfile}" ]; then
		ln -f "$RENUXPKG_SRCDISTDIR/by_sha256/${cksum}_${curfile}" "$distfile"
...
try_mirrors() {
	...
	for mirror in $mirror_list; do
		...
		if [[ "$mirror" == *sources.renux.* ]]; then
			# For sources.renux.* append the subdirectory
...
hook() {
	local srcdir="$RENUXPKG_SRCDISTDIR/$pkgname-$version"
		...
		for i in ${_file_idxs["$f"]}; do
			if [[ -f $distfile ]]; then
				...
				if [[ $cksum = $filesum ]]; then
					...
					find ${RENUXPKG_SRCDISTDIR} -inum ${inode} -delete -print
				fi
			fi
			dfcount=$((dfcount + 1))
		done
	...
	for curfile in ${!_file_idxs[@]}; do
		...
		if try_mirrors "$curfile" "$distfile" "${_checksums[$i]}" "${_distfiles[$i]}" "$RENUXPKG_DISTFILES_MIRROR"; then
			continue
		fi

		if ! try_urls "$curfile"; then
			if try_mirrors "$curfile" "$distfile" "${_checksums[$i]}" "${_distfiles[$i]}" "$RENUXPKG_DISTFILES_FALLBACK"; then
				continue
			fi
