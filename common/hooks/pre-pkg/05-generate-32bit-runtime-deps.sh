hook() {
	local destdir32=${RENUXPKG_DESTDIR}/${pkgname}-32bit-${version}
	...
	: > ${RENUXPKG_STATEDIR}/${pkgname}-32bit-rdeps

	if [ -s "${RENUXPKG_STATEDIR}/${pkgname}-rdeps" ]; then
		if [ -n "$lib32depends" ]; then
			_deps="${lib32depends}"
		else
			_deps="$(<${RENUXPKG_STATEDIR}/${pkgname}-rdeps)"
		fi
		for f in ${_deps}; do
			unset found pkgn pkgv _shprovides

			pkgn="$($RENUXPKG_UHELPER_CMD getpkgdepname $f)"
			if [ -z "${pkgn}" ]; then
				pkgn="$($RENUXPKG_UHELPER_CMD getpkgname $f)"
				if [ -z "${pkgn}" ]; then
					msg_error "$pkgver: invalid dependency $f\n"
				fi
				pkgv="-$($RENUXPKG_UHELPER_CMD getpkgversion ${f})"
			else
				pkgv="$($RENUXPKG_UHELPER_CMD getpkgdepversion ${f})"
			fi
			# If dependency is a development pkg switch it to 32bit.
			if [[ $pkgn == *-devel ]]; then
				echo "   RDEP: $f -> ${pkgn}-32bit${pkgv} (development)"
				printf "${pkgn}-32bit${pkgv} " >> ${RENUXPKG_STATEDIR}/${pkgname}-32bit-rdeps
				continue
			fi
			...
			if [ -z "$found" ]; then
				...
				# via binpkgs.
				_shprovides="$($RENUXPKG_QUERY_CMD -R --property=shlib-provides "$pkgn")"
				if [ -n "${_shprovides}" ]; then
					echo "   RDEP: $f -> ${pkgn}-32bit${pkgv} (shlib-provides)"
					printf "${pkgn}-32bit${pkgv} " >> ${RENUXPKG_STATEDIR}/${pkgname}-32bit-rdeps
				else
					echo "   RDEP: $f -> ${pkgn}${pkgv} (no shlib-provides)"
					printf "${pkgn}${pkgv} " >> ${RENUXPKG_STATEDIR}/${pkgname}-32bit-rdeps
				fi
			else
				if [ -s "${RENUXPKG_STATEDIR}/${pkgn}-shlib-provides" ]; then
	...
	if [[ $pkgn == *-devel ]]; then
		echo "   RDEP: ${pkgver}"
		printf "${pkgver} " >> ${RENUXPKG_STATEDIR}/${pkgname}-32bit-rdeps
	fi
	printf "\n" >> ${RENUXPKG_STATEDIR}/${pkgname}-32bit-rdeps
}
