...
#	- Generates rdeps file with run-time dependencies for rpkg-create(1)
#	- Generates shlib-requires file for rpkg-create(1)

add_rundep() {
    local dep="$1" i= rpkgdep= _depname= found=

    _depname="$($RENUXPKG_UHELPER_CMD getpkgdepname ${dep} 2>/dev/null)"
    if [ -z "${_depname}" ]; then
        _depname="$($RENUXPKG_UHELPER_CMD getpkgname ${dep} 2>/dev/null)"
    fi

    for i in ${run_depends}; do
        rpkgdep="$($RENUXPKG_UHELPER_CMD getpkgdepname $i 2>/dev/null)"
        if [ -z "$rpkgdep" ]; then
            rpkgdep="$($RENUXPKG_UHELPER_CMD getpkgname $i 2>/dev/null)"
        fi
        if [ "${rpkgdep}" != "${_depname}" ]; then
            continue
        fi
        $RENUXPKG_UHELPER_CMD cmpver "$i" "$dep"
        rval=$?
        if [ $rval -eq 255 ]; then
...
store_pkgdestdir_rundeps() {
        if [ -n "$run_depends" ]; then
            for f in ${run_depends}; do
                ...
                if [ -z "$($RENUXPKG_UHELPER_CMD getpkgdepname ${_curdep} 2>/dev/null)" -a \
                     -z "$($RENUXPKG_UHELPER_CMD getpkgname ${_curdep} 2>/dev/null)" ]; then
                    _curdep="${_curdep}>=0"
                fi
                printf "%s " "${_curdep}"
            done > "${RENUXPKG_STATEDIR}/${pkgname}-rdeps"
        fi
}
...
hook() {
    ...
    local _shlib_dir="${RENUXPKG_STATEDIR}/shlib-provides"
    local _shlibtmp

    local Qt_6_PRIVATE_API=6.11.1

    # Disable trap on ERR, rpkg-uhelper cmd might return error... but not something
    # to be worried about because if there are broken shlibs this hook returns
    # error via msg_error().
    trap - ERR

    mapshlibs=$RENUXPKG_COMMONDIR/shlibs
    ...
    for f in ${verify_deps}; do
        ...
        *" $f "*)
        ...
        # If this library is provided by a subpkg of sourcepkg, use that subpkg
            ...
            _rdep="$(awk -v sl="$f" -v arch="$RENUXPKG_TARGET_MACHINE" '$1 == sl && ($3 == "" || $3 == "ignore" || $3 == arch) { print $2; exit; }' "$mapshlibs")"
            ...
            _pkgname=$($RENUXPKG_UHELPER_CMD getpkgname "${_rdep}" 2>/dev/null)
            _rdepver=$($RENUXPKG_UHELPER_CMD getpkgversion "${_rdep}" 2>/dev/null)
    ...
    if [ -n "${sorequires}" ]; then
        echo "${sorequires}" | xargs -n1 | sort | xargs > ${RENUXPKG_STATEDIR}/${pkgname}-shlib-requires
    fi
}
