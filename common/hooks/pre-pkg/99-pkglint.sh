...
hook() {
	...
	local conflictRev= ignore= found= mapshlibs=$RENUXPKG_COMMONDIR/shlibs
	local emptypkg=yes
	...
	if [ ! -s "${RENUXPKG_STATEDIR}/${pkgname}-shlib-provides" ]; then
		return 0
	fi

	for filename in $(<"${RENUXPKG_STATEDIR}/${pkgname}-shlib-provides"); do
		rev=${filename#*.so.}
		libname=${filename%.so*}
		...
		grep -E "${_pattern}" $mapshlibs | { \
			while read -r conflictFile conflictPkg ignore; do
				...
				if [ -n "$ignore" -a "$ignore" != "$RENUXPKG_TARGET_MACHINE" ]; then
					continue
				...
				_revdeps=$($RENUXPKG_QUERY_XCMD -Rs ${libname}.so -p shlib-requires|cut -d ' ' -f1)
			...
			for f in ${provides}; do
				_vpkgname="$($RENUXPKG_UHELPER_CMD getpkgname ${f} 2>/dev/null)"
				_spkgname="$(grep "^${filename}" $mapshlibs | cut -d ' ' -f2)"
				_libpkgname="$($RENUXPKG_UHELPER_CMD getpkgname ${_spkgname} 2>/dev/null)"
