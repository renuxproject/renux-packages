...
for f in $RENUXPKG_COMMONDIR/environment/fetch/*.sh; do
    source_file "$f"
done

RENUXPKG_FETCH_DONE="${RENUXPKG_STATEDIR}/${sourcepkg}_${RENUXPKG_CROSS_BUILD}_fetch_done"

if [ -f "$RENUXPKG_FETCH_DONE" ]; then
    exit 0
fi
...
cd ${RENUXPKG_BUILDDIR} || msg_error "$pkgver: cannot access wrksrc directory [$wrksrc]\n"
