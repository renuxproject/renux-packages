...
for f in $RENUXPKG_COMMONDIR/environment/extract/*.sh; do
    source_file "$f"
done

RENUXPKG_EXTRACT_DONE="${RENUXPKG_STATEDIR}/${sourcepkg}_${RENUXPKG_CROSS_BUILD}_extract_done"

if [ -f $RENUXPKG_EXTRACT_DONE ]; then
    exit 0
fi
...
if declare -f do_extract >/dev/null; then
    ...
    if [ -n "$build_style" ]; then
        if [ ! -r $RENUXPKG_BUILDSTYLEDIR/${build_style}.sh ]; then
            msg_error "$pkgver: cannot find build helper $RENUXPKG_BUILDSTYLEDIR/${build_style}.sh!\n"
        fi
