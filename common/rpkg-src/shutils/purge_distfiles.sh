...
purge_distfiles() {
	readonly HASHLEN=64
	if [ -z "$RENUXPKG_SRCDISTDIR" ]; then
		msg_error "The variable \$RENUXPKG_SRCDISTDIR is not set."
		exit 1
	fi
	#
	...
	#
	# Collect inodes of all distfiles in $RENUXPKG_SRCDISTDIR
	#
	declare -A inodes
	distfiles=($RENUXPKG_SRCDISTDIR/*/*)
	max=${#distfiles[@]}
	if [ -z "$max" ]; then
		msg_error "No distfiles files found in '$RENUXPKG_SRCDISTDIR'"
		exit 1
	fi
	cur=0
	percent=-1
	...
	hashes=($RENUXPKG_SRCDISTDIR/by_sha256/*)
