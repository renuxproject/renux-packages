...
update_check() {
    ...
    local update_override=$RENUXPKG_SRCPKGDIR/$RENUXPKG_TARGET_PKG/update
    ...
    local curlargs=(
        -A "rpkg-src-update-check/$RENUXPKG_SRC_VERSION"
        --max-time 10 --compressed -Lsk
    )

    pkgname=${pkgname#kf6-}

    # RENUXPKG_UPDATE_CHECK_VERBOSE is the old way to show verbose messages
    [ "$RENUXPKG_UPDATE_CHECK_VERBOSE" ] && RENUXPKG_VERBOSE="$RENUXPKG_UPDATE_CHECK_VERBOSE"

    if [ -r $update_override ]; then
        . $update_override
        msg_verbose "using $RENUXPKG_TARGET_PKG/update overrides\n"
        if [ -n "$disabled" ]; then
    ...
    while IFS= read -r found_version; do
        ...
        if $consider; then
            rpkg-uhelper cmpver "$original_pkgname-${version}_1" \
