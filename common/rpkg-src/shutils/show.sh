...
show_pkg_deps() {
    [ -f "${RENUXPKG_STATEDIR}/${pkgname}-rdeps" ] && cat "${RENUXPKG_STATEDIR}/${pkgname}-rdeps"
}
...
show_avail() {
    check_pkg_arch "$RENUXPKG_CROSS_BUILD" 2>/dev/null
}
...
show_eval_dep() {
    ...
    if [ ! -f $RENUXPKG_SRCPKGDIR/$_dep/template ]; then
    ...
    [[ $found ]] && return
    _srcpkg=$(readlink -f ${RENUXPKG_SRCPKGDIR}/${_dep})
    _srcpkg=${_srcpkg##*/}
    echo $_srcpkg
}
...
show_pkg_build_options() {
    ...
    source $RENUXPKG_COMMONDIR/options.description
...
show_pkg_shlib_provides() {
    [ -f "${RENUXPKG_STATEDIR}/${pkgname}-shlib-provides" ] && cat "${RENUXPKG_STATEDIR}/${pkgname}-shlib-provides"
}

show_pkg_shlib_requires() {
    [ -f "${RENUXPKG_STATEDIR}/${pkgname}-shlib-requires" ] && cat "${RENUXPKG_STATEDIR}/${pkgname}-shlib-requires"
}
