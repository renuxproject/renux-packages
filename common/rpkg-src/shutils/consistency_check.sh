...
consistency_check_existing () {
    while IFS=" " read -r dep origname deplabel; do
        [ -f "$RENUXPKG_SRCPKGDIR/$dep/template" ] && continue
        case "$deplabel" in
...
consistency_convert_pkgname () {
    ...
    while IFS=" " read -r dep origname deplabel; do
        ...
        if pkgname=$(rpkg-uhelper getpkgname "$dep" 2> /dev/null) && \
            version=$(rpkg-uhelper getpkgversion "$dep" 2> /dev/null) && \
            revision=$(rpkg-uhelper getpkgrevision "$dep" 2> /dev/null); then
...
consistency_check_smart () {
    ...
    while IFS=" " read -r depdef origname deplabel; do
        ...
        dep=$(rpkg-uhelper getpkgdepname "$depdef")

        if [ ! -f "$RENUXPKG_SRCPKGDIR/$dep/template" ]; then
            msg_warn "unsatisfied $deplabel in $origname: $dep does not exist\n";
            continue
        fi
        (
            RENUXPKG_TARGET_PKG=$dep
            read_pkg
            rpkg-uhelper pkgmatch "$depdef" "${pkgname}-${version}_${revision}" && return
...
consistency_check() {
    local pkg= pkgname=
    for pkg in "$RENUXPKG_SRCPKGDIR"/*/template; do
        pkg=${pkg%/*}
        RENUXPKG_TARGET_PKG=${pkg##*/}
        (
            read_pkg
            [ "$depends" ] && printf "%s $pkgname depends\n" $depends
            [ "$conflicts" ] && printf "%s $pkgname conflicts\n" $conflicts
            [ -L "$RENUXPKG_SRCPKGDIR/$RENUXPKG_TARGET_PKG" ] && return
