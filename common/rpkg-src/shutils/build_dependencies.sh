...
setup_pkg_depends() {
    ...
    for j in ${depends}; do
        ...
        if [[ ${_rpkgname} == virtual ]]; then
            _pkgname=$(rpkg-uhelper getpkgname $_depname 2>/dev/null)
            [ -z "$_pkgname" ] && _pkgname="$_depname"
            if [ -s ${RENUXPKG_DISTDIR}/etc/virtual ]; then
                foo=$(grep -E "^${_pkgname}[[:blank:]]" ${RENUXPKG_DISTDIR}/etc/virtual|cut -d ' ' -f2)
            elif [ -s ${RENUXPKG_DISTDIR}/etc/defaults.virtual ]; then
                foo=$(grep -E "^${_pkgname}[[:blank:]]" ${RENUXPKG_DISTDIR}/etc/defaults.virtual|cut -d ' ' -f2)
            fi
            if [ -z "$foo" ]; then
            ...
            foo="$($RENUXPKG_UHELPER_CMD getpkgdepname ${_depname} 2>/dev/null)"
            if [ -z "$foo" ]; then
                foo="$($RENUXPKG_UHELPER_CMD getpkgname ${_depname} 2>/dev/null)"
                [ -z "$foo" ] && foo="${_depname}"
            fi
            [[ $out ]] && echo "$foo"
        fi
...
# EBUSY   (16): package 'renuxpkg' needs to be updated.
# EEXIST  (17): file conflicts in transaction (RENUXPKG_FLAG_IGNORE_FILE_CONFLICTS unset)
...
install_pkg_from_repos() {
    ...
    mkdir -p $RENUXPKG_STATEDIR
    tmplogf=${RENUXPKG_STATEDIR}/renuxpkg_${RENUXPKG_TARGET_MACHINE}_bdep_${pkg}.log

    cmd=$RENUXPKG_INSTALL_CMD
    [[ $cross ]] && cmd=$RENUXPKG_INSTALL_XCMD
    $cmd -Ay "$@" >$tmplogf 2>&1
    rval=$?
    ...
    case "$rval" in
        0) # success, check if there are errors.
           ...
           ;;
        *)
           [ -z "$RENUXPKG_KEEP_ALL" ] && remove_pkg_autodeps
...
check_installed_pkg() {
    ...
    pkgn="$($RENUXPKG_UHELPER_CMD getpkgname ${pkg})"
    [ -z "$pkgn" ] && return 2

    uhelper=$RENUXPKG_UHELPER_CMD
    [[ $cross ]] && uhelper=$RENUXPKG_UHELPER_XCMD
    iver="$($uhelper version $pkgn)"
    if [ $? -eq 0 -a -n "$iver" ]; then
        $RENUXPKG_CMPVER_CMD "${pkgn}-${iver}" "${pkg}"
...
#
skip_check_step() {
    [ -z "$RENUXPKG_CHECK_PKGS" ] ||
        [ "$RENUXPKG_CROSS_BUILD" ] ||
        [ "$make_check" = ci-skip  -a "$RENUXPKG_BUILD_ENVIRONMENT" = renux-packages-ci ] ||
        [ "$make_check" = extended -a "$RENUXPKG_CHECK_PKGS" != full ] ||
        [ "$make_check" = no ]
}
...
install_pkg_deps() {
    ...
    if [ "$pkg" != "$targetpkg" ]; then
        msg_normal "$pkgver: building${style} (dependency of $targetpkg) for $RENUXPKG_TARGET_MACHINE...\n"
    else
        msg_normal "$pkgver: building${style} for $RENUXPKG_TARGET_MACHINE...\n"
    fi
    ...
    if [[ ${hostmakedepends} ]]; then
        ...
        for f in ${hostmakedepends}; do
            if [ -f $RENUXPKG_SRCPKGDIR/$f/template ]; then
                templates+=" $f"
                continue
            fi
            local _repourl=$($RENUXPKG_QUERY_CMD -R -prepository "$f" 2>/dev/null)
            if [ "$_repourl" ]; then
        ...
        done < <($RENUXPKG_CHECKVERS_CMD -D $RENUXPKG_DISTDIR -sm $templates)
    fi
    ...
    if [[ ${makedepends} ]]; then
        ...
        for f in ${makedepends}; do
            ...
            local _repourl=$($RENUXPKG_QUERY_XCMD -R -prepository "$f" 2>/dev/null)
            if [ "$_repourl" ]; then
        ...
        done < <($RENUXPKG_CHECKVERS_XCMD -D $RENUXPKG_DISTDIR -sm $templates)
    fi
    ...
    if [ -n "$RENUXPKG_BUILD_ONLY_ONE_PKG" ]; then
    ...
    for i in ${host_missing_deps[@]}; do
        ...
        (
        curpkgdepname=$($RENUXPKG_UHELPER_CMD getpkgname "$i" 2>/dev/null)
        setup_pkg $curpkgdepname
        # do not check when building dependencies, except for "full" (-K)
        [ "$RENUXPKG_CHECK_PKGS" == full ] || unset RENUXPKG_CHECK_PKGS
        exec env RENUXPKG_DEPENDENCY=1 RENUXPKG_BINPKG_EXISTS=1 RENUXPKG_DEPENDS_CHAIN="$RENUXPKG_DEPENDS_CHAIN, $sourcepkg(host)" \
            $RENUXPKG_LIBEXECDIR/build.sh $sourcepkg $pkg $target $cross_prepare || exit $?
        ) || exit $?
        host_binpkg_deps+=("$i")
    done
    ...
    for i in ${missing_deps[@]}; do
        ...
        # do not check when building dependencies, except for "full" (-K)
        [ "$RENUXPKG_CHECK_PKGS" == full ] || unset RENUXPKG_CHECK_PKGS
        exec env RENUXPKG_DEPENDENCY=1 RENUXPKG_BINPKG_EXISTS=1 RENUXPKG_DEPENDS_CHAIN="$RENUXPKG_DEPENDS_CHAIN, $sourcepkg(${cross:-host})" \
            $RENUXPKG_LIBEXECDIR/build.sh $sourcepkg $pkg $target $cross $cross_prepare || exit $?
        ) || exit $?
        binpkg_deps+=("$i")
    done
    ...
    for i in ${missing_rdeps[@]}; do
        # packages not found in repos, install from source.
        (
        curpkgdepname=$($RENUXPKG_UHELPER_CMD getpkgdepname "$i" 2>/dev/null)
